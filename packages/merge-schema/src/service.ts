...
import { DocumentNode, InterfaceTypeDefinitionNode, ObjectTypeDefinitionNode, parse, StringValueNode } from "graphql";
type TypeName = string;
type Description = string;
export class MergeSchemaService {
  private descriptionMap: Map<TypeName, Description> = new Map<TypeName, Description>();
  ...
  private cacheDescriptions(type: DocumentNode): void {
    for (const definition of this.getAllObjectDefinitionsByDocumentNode(type)) {
      ...
      if (this.descriptionMap.has(definition.name.value)) {
        description = [this.descriptionMap.get(definition.name.value), definition.name.value].join("\n");
      }
  ...
  public applyInterfaces(type: DocumentNode): void {
    for (const definition of this.getAllObjectDefinitionsByDocumentNode(type)) {
      if (this.interfaceMap.has(definition.name.value)) {
        const interfaces = Object.assign([], definition.interfaces || []) as InterfaceTypeDefinitionNode[];
        const cachedInterfaces = this.interfaceMap.get(definition.name.value);
        if (!Array.isArray(cachedInterfaces)) {
          continue;
        }
        for (const cachedInterface of cachedInterfaces) {
          if (!interfaces.find((i: InterfaceTypeDefinitionNode) => i.name.value === cachedInterface.name.value)) {
  ...
  private getAllObjectDefinitionsByDocumentNode(documentNode: DocumentNode): ObjectTypeDefinitionNode[] {
    ...
    for (const objOrUnion of getObjectOrUnionTypeDefinitions(documentNode)) {
      if (isUnionType(objOrUnion)) {
        definitions.push(...getObjectTypeDefinitionsFromUnion(documentNode, objOrUnion));
      } else {
